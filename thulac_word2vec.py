...
class Get_Sentences(object):
    ...
    Yield:
        word:a list of word cut by thulac
    ...
    def __init__(self, filenames):
        ...
        self.thu = thulac.thulac(user_dict="general_dict.txt", seg_only=True)
    ...
    def sentence2words(self, sentence, stopWords=False, stopWords_set=None):
        """ 
        split a sentence into words based on thulac
        """
        text = self.thu.cut(sentence, text=True)
...
def train_word2vec(filenames):
    ...
    model = word2vec.Word2Vec(sentences, size=100, window=5, min_count=6, max_vocab_size=config.vocab_size, workers=10)
